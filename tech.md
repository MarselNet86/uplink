...
Changelog:
- `v8` - **дефолт Hysteria2 снова `acme-domain` на автодомене `sslip.io`** (частичный откат решения v2 и фикса BUG-07): при выключённом чекбоксе домен выводится из введённого адреса через `deriveAutoDomain`, email - `admin@<домен>`. `self-signed` остаётся запасным путём для случаев, когда домен вывести не из чего (IPv6 и всё, что не IPv4 и не FQDN). Причина решения: клиенты на sing-box и нативный hysteria отвергают пиннинг и требуют настоящий сертификат (раздел 5.9), а требовать от пользователя собственный домен - ровно то, чего продукт старается избежать. Изменены: раздел 4, `ConnectForm.tsx`, `formValidation.ts`. Маппинг формы в `DeployParams` вынесен из компонента в чистую функцию `buildDeployParams` и покрыт тестами - именно этот маппинг ломался в BUG-07, и внутри компонента его нечем было проверить.
  **Известный риск, принят осознанно:** `sslip.io` отсутствует в Public Suffix List (проверено по актуальному списку, 14346 записей). Let's Encrypt считает лимит «50 сертификатов на Registered Domain в неделю» по PSL, поэтому registered domain для `<ip>.sslip.io` - это `sslip.io` целиком, одно ведро на всех пользователей сервиса в мире. При исчерпании квоты выпуск падает. Чтобы это не выглядело как «сервис не слушает порт», `Hysteria2Installer.acmeFailureHint()` читает журнал юнита и при `rateLimited`/`too many certificates` отдаёт в `AppError.hint` прямое указание перейти на свой домен.
  Также осознанно принимаются: публикация `<ip>.sslip.io` в CT-логах (адрес сервера становится публично индексируемым), необходимость открытого `80/tcp` под http-01, ожидание выпуска до 180 с и зависимость от доступности DNS `sslip.io`.
- `v7` - **Один прогон на сессию.** `install:start` и `protocols:remove` больше не запускают второй пайплайн поверх той же SSH-сессии: повторный вызов возвращает `RunHandle` уже идущего прогона (тип ответа не меняется, ошибки нет - повторный клик это тот же запрос). Причина: с v6 `install:start` до возврата ждёт `ProtocolDetector` в `assertInstallable`, а renderer уходит со шага 2 только по первому `progress:event` - окно между кликом и переходом достаточно широкое, чтобы двойным кликом по Install запустить два пайплайна по одной сессии, а они пишут одни и те же конфиги и дёргают одни и те же юниты. Добавлен `claimSessionRun`, освобождение повешено на `clearRun` (в том числе при отказе `assertInstallable`, иначе отклонённая установка заклинивала бы сессию). В renderer кнопка Install на время вызова блокируется и показывает `Starting...`. Изменены: `pipeline/runRegistry.ts`, `installStart.ts`, `protocolsRemove.ts`, `App.tsx`, `SelectStep.tsx`.
- `v7` - **`protocols:refresh` перечитывает только `ports`**, накладывая результат на отчёт из `ssh:check`, который теперь хранится в `sessionRegistry` вместе с сессией и хостом. Полный прогон раздела 5.4, добавленный в v6, вешал на кнопку Done curl до `github.com` и семь лишних SSH round-trip-ов ради проверок, которые установка или удаление изменить не могут: privileges, distro, arch и systemd не меняются при живой сессии, outbound и apt-lock от прогона не зависят. Отчёт шага 2 в любом случае совещательный - перед реальными действиями `installStart` прогоняет полный preflight на сервере, и решает именно он. `Preflight.checkPorts` стал публичным. Изменены: `sessionRegistry.ts`, `sshCheck.ts`, `protocolsRefresh.ts`, `Preflight.ts`.
...
- `v4` - по результатам первого боевого прогона на реальном сервере. **Донор Reality больше не выбирается по одному лишь `xray tls ping`:** проверка дополнена измерением размера цепочки сертификатов донора, а `www.microsoft.com` удалён из списка. Причина (воспроизведено и подтверждено отладкой REALITY `show: true`): у него RSA-цепочка ~8 КБ, она не помещается в буфер, которым REALITY ретранслирует рукопожатие донора, и соединение рвётся уже **после** успешной авторизации клиента (`hs.c.conn == conn: true`, затем `Certificate: 8273` при 5360 доступных и `isHandshakeComplete: false`). `xray tls ping` при этом возвращает успех, то есть старая проверка такой донор пропускала, а он стоял первым в списке - поэтому не работал ни один установленный сервер. Изменены: `domain/RealityDonors.ts` (состав константы), шаг X4 (критерий отбора). Добавлены `DeployParams.realitySni` и `DeployParams.hysteriaSni` - пользовательский SNI вместо встроенных значений (частичный откат решения v2 «поле SNI убрано целиком»: значения по умолчанию сохраняются, поле опциональное и лежит в свёрнутом блоке). Добавлены IPC-каналы `clipboard:write` и `protocols:refresh`.
...
Свёрнутый блок **«Domain (optional)»**, по умолчанию закрыт, разворачивается по клику:
...
Если чекбокс включён - `acme-domain` на домене пользователя, с требованием, чтобы A/AAAA-запись указывала на IP сервера. Если выключен (дефолт, **изменено в v8**) - домен выводится из уже введённого адреса функцией `deriveAutoDomain`: IPv4 получает суффикс `sslip.io` (`1.2.3.4.sslip.io` резолвится в `1.2.3.4` без регистрации), хост-домен переиспользуется как есть. Режим при этом всё равно `acme-domain`, email подставляется как `admin@<домен>`. `self-signed` остаётся только запасным путём - когда домен вывести не из чего (IPv6 и всё, что не IPv4 и не FQDN). Маппинг формы в `DeployParams` живёт в чистой функции `buildDeployParams`, не внутри компонента. Переключение режима валится в тип `TlsMode` (раздел 7).
...
- `installed` - чекбокс задизейблен, рядом бейдж `Installed` и ссылка `Manage`;
- `broken` - чекбокс задизейблен, бейдж `Found, not running`, ссылка `Manage`.
...
Открывается по `Manage` или при попытке установить уже установленный протокол. Заголовок: «Protocol is already installed on the server». Варианты:
- **Reinstall** - полное удаление + чистая установка с новыми ключами. Явное предупреждение: старые ссылки перестанут работать.
- **Remove** - только удаление.
- **Cancel**.
...
- Под ним список этапов (`StepId` -> человекочитаемое название на английском, v5) со статусами `pending` / `running` / `done` / `failed` / `skipped`.
- Текущий этап подсвечен, справа проценты.
- Кнопка **Cancel** активна до начала записи конфигов; после точки невозврата заменяется на пояснение «Cancelling may leave the server in an intermediate state».
- Сырой лог скрыт. При ошибке появляется раскрывающийся блок `Diagnostics` с последними строками stderr, прогнанными через `redact()`.
...
- кнопка **Copy** (по клику - `Copied` на 2 секунды);
- кнопка **Copy all** внизу;
- кнопка **Save to file** (`.txt`, диалог Electron), по явному действию пользователя.
...
```
Дополнительно: `grep -q '"security"[[:space:]]*:[[:space:]]*"reality"' /usr/local/etc/xray/config.json` чтобы отличить Reality от произвольной конфигурации Xray. Если Xray стоит, но без Reality - статус `foreign`, установка запрещена, в модалке только «Remove» с предупреждением, что конфиг чужой.
...
**Шаг X4. Выбор и проверка донорского домена.** По умолчанию пользователь донора не вводит: приложение хранит встроенный список кандидатов (`domain/RealityDonors.ts`, константа, правится только через bump версии ядра). Если пользователь заполнил `DeployParams.realitySni`, список заменяется этим единственным кандидатом (он проходит те же проверки, а не принимается на веру).

Кандидат обязан пройти **две** проверки, обе обязательны:

1. TLS 1.3 и HTTP/2 - `xray tls ping <candidate>` (успех по коду возврата);
2. **размер цепочки сертификатов** - не больше `MAX_DONOR_CERT_BYTES`:
```
echo | openssl s_client -connect <candidate>:443 -servername <candidate> -showcerts \
  | awk '/BEGIN CERT/,/END CERT/' | wc -c
```

Вторая проверка не косметическая. REALITY ретранслирует рукопожатие донора через буфер ограниченного размера, и слишком длинная цепочка обрывает соединение **уже после** успешной авторизации клиента - снаружи это выглядит как «сервис работает, порт слушается, но трафик не идёт». Первая проверка это не ловит: `xray tls ping` возвращает успех и для непригодных доноров. Замеры на реальном сервере (Xray 26.3.27): `www.microsoft.com` 8126 Б - рвётся; `dl.google.com` 6728 Б, `www.amazon.com` 6059 Б, `www.lovelive-anime.jp` 5311 Б, `www.apple.com` 4484 Б, `www.swift.com` 4127 Б, `www.cloudflare.com` 3540 Б, `one.one.one.one` 3233 Б - все работают. Порог выставлен консервативно между наибольшим рабочим и наименьшим сломанным замером; при недоступном `openssl` проверка пропускается с предупреждением, а не роняет установку.

Первый прошедший обе проверки кандидат становится `REALITY_SNI`, используется и в `dest`, и в `serverNames`. Если ни один кандидат не прошёл - `E_NO_REALITY_DONOR`, установка Reality не выполняется, Hysteria2 (если выбран) продолжает независимо. Выбранный донор попадает в `RunResult.warnings` для прозрачности.
...
export interface Step {
  id: StepId;
  title: string;        // en, показывается пользователю
...
export interface AppError {
  code: ErrorCode;
  message: string;               // en, для пользователя
...
| `Collapsible` | блок «Diagnostics» | shadcn |
...
- Строки, показываемые пользователю, лежат в renderer. Main возвращает коды и структурированные данные, не готовые английские фразы (исключение: `title` шага, он часть контракта `StepView`).
...
**Стадия 0. Скелет.**
Инициализация electron-vite + React + Tailwind, security-хардненинг окна, `src/shared` с типами и схемами, регистрация одного демо-канала IPC, примитивы UI отрисованы в демонстрационном роуте, vitest настроен и гоняет один тривиальный тест, CI зелёный, `electron-builder` собирает пустое приложение под все три платформы. (Демо-роут со всеми примитивами удалён после стадии 4 — он выполнил свою задачу и остался бы мёртвым кодом; вместе с ним ушли `RouteId`/`route` из стора, баррель `ui/index.ts` и не имевший других потребителей `ui/Card.tsx`.)
...
- примитивы из раздела 9 отрендерены в демо-роуте;
- `vitest run` проходит;
