...
```
Официальный скрипт кладёт бинарь в `/usr/local/bin/xray`, конфиг в `/usr/local/etc/xray/`, юниты `xray.service` и `xray@.service` в `/etc/systemd/system/`, geodata в `/usr/local/share/xray/`. Проверка после шага: `xray version` и `test -x /usr/local/bin/xray`. Скрипт по умолчанию ставит сервис под пользователем `nobody`, что важно для прав на конфиг.
...
```
Срок 100 лет осознанно: самоподписанный сертификат никем не проверяется по цепочке доверия, ротация тут не даёт защиты, а только добавляет точку отказа (протухший серт = недоступный сервис). Отпечаток снимается сразу после генерации:
```
...
Дизайн-токены (цвета, радиусы, spacing) объявляются один раз в `styles/index.css` через CSS-переменные Tailwind v4. Ни один компонент не хардкодит hex.

Окно: фиксированное, 480x720, без ресайза по ширине, тёмная тема по умолчанию, светлая по системной настройке.
